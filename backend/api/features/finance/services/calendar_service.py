...
class CalendarService:
    def generate_calendar_data(
        ...
        # Get all data
        bills = list(RecurringBill.objects.filter(user=user))
        paychecks = list(Paycheck.objects.filter(user=user))
        ...
        while current_date <= calc_end_date:
            ...
            day_bills = self._get_bills_for_date(bills, current_date, bill_payments) if should_calculate else []
                ...
                'bills': [
                    {
                        ...
                        'color': bill.color,
                        'total': bill.total,
                        'amountPaid': bill_payments.get(bill.id, Decimal('0.00')) if bill.total else None
                    }
                    for bill in day_bills
                ],
                'paychecks': [
    ...
    def _get_paychecks_for_date(self, paychecks: List[Paycheck], target_date: date) -> List[Paycheck]:
    ...
    def _is_paycheck_on_date(self, paycheck: Paycheck, target_date: date) -> bool:
        ...
        if frequency == 'once':
            ...
            if paycheck.day_of_week is not None:
                return target_date.weekday() == paycheck.day_of_week and target_date >= paycheck.date
